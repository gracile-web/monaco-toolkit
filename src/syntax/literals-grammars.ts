...
import { patchedHtmlGrammar } from './html-custom.js';

const lit = {
	litHtml: {
		htmlCssTaggedTemplatePJson: {
			injectTo: [
				'source.js',
				'source.js.jsx',
				'source.jsx',
		...
		htmlCssTaggedTemplateLang: {
			fileTypes: [],
			injectionSelector:
				'L:source.js -comment -(string -meta.embedded), L:source.jsx -comment -(string -meta.embedded),  L:source.js.jsx -comment -(string -meta.embedded), L:source.ts -comment -(string -meta.embedded), L:source.tsx -comment -(string -meta.embedded)',
			injections: {
				'L:source': {
					patterns: [
						{
			...
			patterns: [
				...
				// },
				{
					...
					beginCaptures: {
						0: {
							name: 'punctuation.definition.interpolation.begin.bracket.curly.ts punctuation.definition.template-expression.begin.ts',
							// name: 'keyword',
						},
					},
					end: '\\}',
					endCaptures: {
						0: {
							name: 'punctuation.definition.interpolation.end.bracket.curly.ts punctuation.definition.template-expression.end.ts',
							// name: 'keyword', // NOTE: Monaco prefers keyword for visible scope coloring
						},
					},
					patterns: [
						// {
		...
		stringInjectionPjson: {
			...
			embeddedLanguages: {
				...
				'meta.template.expression.tsx': 'tsx',
			},
		},
		...
		cssStyledComponentsPjson: {
			...
			embeddedLanguages: {
				...
				'meta.embedded.line.tsx': 'tsx',
			},
		},
		cssStyledComponentsLang: {
		...
		styleInjectionLang: {
			fileTypes: [],
			injectionSelector: 'L:meta.embedded.block.html meta.property-value.css',
			patterns: [
				{
					include: 'source.js#template-substitution-element',
				},
...
export const litShikiLanguages: Record<
	string,
	DynamicImportLanguageRegistration
> = {
	'lit-html': async () => ({
		default: [
			{
				...lit.litHtml.htmlCssTaggedTemplatePJson,
				...lit.litHtml.htmlCssTaggedTemplateLang,
				name: 'htmlCssTaggedTemplate',
				repository: {} as any,
			},

			{
				name: 'comment-markers',
				scopeName: 'comment.markers',
				...
				repository: {
					'comment-mark': {
						patterns: [
							{
								match: '//\\s*(MARK:)(.*)',
								name: 'comment.line.double-slash.mark.full',
								captures: {
									'1': {
										name: 'comment.keyword.mark.ts',
									},
									'2': {
										name: 'comment.mark.trailing.ts',
									},
								},
							},
						],
			...
			// 		'source.tsx',
			// 	],
			// 	scopeName: 'source.ts.cem',

			// 	fileTypes: [],

			// 	injectionSelector: 'L:comment.block.documentation.ts',

			// 	patterns: [
			// 		{
			// 			name: 'keyword.other.documentation.cem',
			// 			match: '@(slot|cssProp|cssPart)',
			// 		},
			// 		{
			// 			name: 'variable.parameter.documentation.cem',
			// 			match: '(?<=@(slot|cssProp|cssPart)\\s)[^\\s]+',
			// 		},
			// 	],
			// },

			{
				...lit.litHtml.stringInjectionPjson,
				...lit.litHtml.stringInjectionLang,
				name: 'stringInjection',
				repository: {} as any,
			},
			{
				...lit.litHtml.cssStyledComponentsPjson,
				...lit.litHtml.cssStyledComponentsLang,
				name: 'cssStyledComponents',
				repository: {} as any,
			} as any as LanguageRegistration, // NOTE: Type mismatches for 0 indexes
			{
				...lit.litHtml.styleInjectionPjson,
				...lit.litHtml.styleInjectionLang,
				name: 'Lit 6',
				repository: {} as any,
			},
			patchedHtmlGrammar as any,
		],
	}),
};
